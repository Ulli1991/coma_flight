...
# snapshot 139 additionally 384^3 + 192^3 inner +-1200 ckpc/h) for gas mass, temperature,
# X-ray proxies, Arepo shock-finder Mach number (max in voxel), shock energy dissipation,
# dark matter mass, r-band star light and g-r colour.  pack_cubes.py turns them into the
# u8.gz files the page loads.  Centre for SNAP != 139: median position of the 3000 most
...
def do_gas(args):
    ...
    if FULL:
        o2 = grids(d, {'gas_m384': mass, 'gas_ed384': ed}, HALF, N2, {'max_mach384': mach}); out.update(o2)
...
def do_stars(args):
    ...
    if FULL:
        out.update(grids(d, {'st_L384': L, 'st_Lc384': L * gr}, HALF, N2))
        mi = np.max(np.abs(d), axis=1) < IH
...
if __name__ == '__main__':
    ...
    print('wrote', OUT + '/raw_%03d.h5' % SNAP, 'keys', sorted(acc), '%.0f s' % (time.time() - t0), flush=True)
